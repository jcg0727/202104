...
public class BoardServiceImpl implements BoardService {
	...
	@Override
	public void regist(BoardVO notice) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {

			int bno = boardDAO.selectBoardSequenceNextValue(session);
			notice.setBno(bno);
			boardDAO.insertBoard(session, notice);
		} finally {
			session.close();
		}
	}
	@Override
	public void modify(BoardVO notice) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {

			boardDAO.updateBoard(session, notice);
		} finally {
			session.close();
		}
	}
